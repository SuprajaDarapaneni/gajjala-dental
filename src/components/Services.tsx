...
const services = [
  ...
  },
  {
    ...
    icon: BabyIcon,
    // image removed to meet your requirement
    color: 'bg-primary/5 text-primary'
  },
];
    ...
    <section id="services" className="py-24 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4 italic tracking-tight">
            Comprehensive Dental Care
          </h2>
          <p className="text-primary/60 font-light text-lg">
            We offer a wide range of specialized dental treatments to ensure your oral health is at its best.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card key={index} className="group transition-all duration-500 border-primary/10 overflow-hidden hover:border-primary/20 hover:shadow-2xl hover:shadow-primary/5 rounded-none flex flex-col">
              ...
              {service.image ? (
                ...
                /* Placeholder for services without images (e.g., Pediatric Dentistry) */
                <div className="h-56 bg-secondary/30 flex items-center justify-center shrink-0 border-b border-primary/5">
                   <service.icon className="w-16 h-16 text-primary/10 group-hover:scale-110 group-hover:text-accent/40 transition-all duration-500" />
                </div>
              )}
              ...
              <CardContent className="p-8 flex-grow flex flex-col">
                ...
                <h3 className="text-2xl font-bold text-primary mb-3 italic">
                  {service.title}
                </h3>
                
                <p className="text-primary/60 mb-6 leading-relaxed font-light flex-grow">
                  {service.description}
                </p>
                
                <Link 
                  to="/treatments" 
                  className="inline-flex items-center text-xs uppercase tracking-[0.2em] font-bold text-primary hover:text-accent transition-colors"
                >
