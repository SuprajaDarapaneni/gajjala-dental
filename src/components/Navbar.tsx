...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      {/* Upper Info Bar - Hidden on Scroll for a clean look */}
      <div className={`hidden lg:block bg-primary text-white/80 py-2 transition-all duration-500 ${isScrolled ? '-translate-y-full opacity-0 h-0' : 'translate-y-0 opacity-100'}`}>
                ...
                <div className="flex flex-col h-full p-8">
                  ...
                  <nav className="flex flex-col gap-6 mt-16">
                    {navLinks.map((link) => (
                      <Link 
                        key={link.name} 
                        to={link.href} 
                        className="text-4xl font-extrabold italic hover:text-accent transition-all hover:translate-x-4 inline-block"
                      >
                        {link.name}
                      </Link>
                    ))}
                    <Link to="/treatments" className="text-4xl font-extrabold italic hover:text-accent transition-all hover:translate-x-4 inline-block">
                      Treatments
                    </Link>
                  </nav>
                  ...
                  <div className="mt-auto pt-12 border-t border-white/10 space-y-6">
                    <div className="grid grid-cols-1 gap-4">
                       <Button asChild className="w-full bg-accent text-primary font-bold py-8 text-lg rounded-2xl shadow-2xl">
                          <Link to="/booking">BOOK NOW</Link>
                       </Button>
                    </div>
                    <div className="flex items-center gap-4 text-white/60">
                      <Clock className="w-5 h-5 text-accent" />
                      <span className="text-xs font-bold tracking-widest uppercase">9 AM - 8 PM Every Day</span>
                    </div>
                  </div>
                </div>
              </SheetContent>
